...
?>
<tr id="bookme-employee-<?php echo esc_attr($member['id']) ?>" data-id="<?php echo esc_attr($member['id']) ?>">
    <td>
        <div class="checkbox">
            <input type="checkbox" id="check_<?php echo esc_attr($member['id']) ?>" value="<?php echo esc_attr($member['id']) ?>"
                   class="bm-check">
            <label for="check_<?php echo esc_attr($member['id']) ?>"><span class="checkbox-icon"></span></label>
        </div>
    </td>
    <td>
    ...
    <td class="staff-services">
        <?php if (!empty($member['services'])) {
            ...
            foreach ($services as $service) {
                echo "<div class='badge badge-primary'>".esc_html($service)."</div>";
            }
            if ($rest > 0)
                echo "<div class='badge badge-primary'>+" . esc_html($rest) . "</div>";
        } else {
